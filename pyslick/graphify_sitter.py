...
# Set working directory context for PySlick / Tree-Sitter
PYSLICK_DIR = r"C:\Users\corte\Documents\do not delete second brain\pyslick"
...
def run_auto_graphify_extract():
    try:
        sys.stderr.write(f"[graphify_sitter] AST result is null. Auto-running graphify extract . --code-only (PySlick context: {PYSLICK_DIR})...\n")
        subprocess.run("graphify extract . --code-only", shell=True, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return True
...
def main(user_query):
    ...
    # AUTO-EXTRACTION TRIGGER: If AST parsing returned null, run graphify extract and retry
    if not ast_res:
